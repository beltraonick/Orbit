-- POST_037_039_PRODUCTION_CHECK.sql — final snapshot after 037, 038 and 039.
...
-- Reproduces the PRE snapshot (CAA section) and verifies every schema addition from 037–039.
WITH
...
report(ord, section, check_name, value, result) AS (
  ...
  UNION ALL SELECT 1, 'PREREQ', 'company_document_settings table (029)', ((to_regclass('public.company_document_settings') IS NOT NULL))::text, CASE WHEN ((to_regclass('public.company_document_settings') IS NOT NULL)) = true THEN 'PASS' ELSE 'STOP' END
  UNION ALL SELECT 2, 'PREREQ', 'settings clock-window / home-period columns (033, 036)',
                   (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='timezone') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='home_period_type') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='mileage_rate_per_mile'))::text, CASE WHEN (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='timezone') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='home_period_type') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='mileage_rate_per_mile')) = true THEN 'PASS' ELSE 'STOP' END
  UNION ALL SELECT 3, 'PREREQ', 'profiles.daily_rate + workers table (028)', (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='profiles' AND column_name='daily_rate') AND (to_regclass('public.workers') IS NOT NULL))::text, CASE WHEN (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='profiles' AND column_name='daily_rate') AND (to_regclass('public.workers') IS NOT NULL)) = true THEN 'PASS' ELSE 'STOP' END
  UNION ALL SELECT 4, 'PREREQ', 'workers.hourly_rate (035)', (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='workers' AND column_name='hourly_rate'))::text, CASE WHEN (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='workers' AND column_name='hourly_rate')) = true THEN 'PASS' ELSE 'STOP' END
  UNION ALL SELECT 5, 'PREREQ', 'time_entries.is_full_day (028) + projects table', (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='time_entries' AND column_name='is_full_day') AND (to_regclass('public.projects') IS NOT NULL))::text, CASE WHEN (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='time_entries' AND column_name='is_full_day') AND (to_regclass('public.projects') IS NOT NULL)) = true THEN 'PASS' ELSE 'STOP' END
  UNION ALL SELECT 6, 'PREREQ', 'gen_random_uuid() available', (EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'gen_random_uuid'))::text, CASE WHEN (EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'gen_random_uuid')) = true THEN 'PASS' ELSE 'STOP' END

  UNION ALL SELECT 10, 'SCHEMA', '037 company_document_settings.pay_system exists', (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='pay_system'))::text, 'INFO'
  UNION ALL SELECT 11, 'SCHEMA', '038 payroll_periods exists', ((to_regclass('public.payroll_periods') IS NOT NULL))::text, 'INFO'
  UNION ALL SELECT 12, 'SCHEMA', '038 payroll_period_entries exists', ((to_regclass('public.payroll_period_entries') IS NOT NULL))::text, 'INFO'
  UNION ALL SELECT 13, 'SCHEMA', '039 manual_compensations exists', ((to_regclass('public.manual_compensations') IS NOT NULL))::text, 'INFO'
  UNION ALL SELECT 14, 'SCHEMA', 'abandoned-branch column company_document_settings.standard_day_hours present (harmless, not used by main)',
                   (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='standard_day_hours'))::text, 'INFO'
  UNION ALL SELECT 15, 'SCHEMA', 'abandoned-branch column company_document_settings.company_pay_mode present (harmless, not used by main)',
                   (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='company_document_settings' AND column_name='company_pay_mode'))::text, 'INFO'
  ...
  UNION ALL SELECT 60, '039', 'manual_compensations exists', ((to_regclass('public.manual_compensations') IS NOT NULL))::text, CASE WHEN ((to_regclass('public.manual_compensations') IS NOT NULL)) = true THEN 'PASS' ELSE 'FAIL' END
  UNION ALL SELECT 61, '039', 'RLS + anon_all policy on manual_compensations',
                   (coalesce((SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.manual_compensations')), false) AND EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='manual_compensations' AND policyname='anon_all'))::text, CASE WHEN (coalesce((SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.manual_compensations')), false) AND EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='manual_compensations' AND policyname='anon_all')) = true THEN 'PASS' ELSE 'FAIL' END
  UNION ALL SELECT 62, '039', 'payroll_period_entries.source_type / source_manual_compensation_id / category exist',
                   (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='payroll_period_entries' AND column_name='source_type') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='payroll_period_entries' AND column_name='source_manual_compensation_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='payroll_period_entries' AND column_name='category'))::text, CASE WHEN (EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='payroll_period_entries' AND column_name='source_type') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='payroll_period_entries' AND column_name='source_manual_compensation_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='payroll_period_entries' AND column_name='category')) = true THEN 'PASS' ELSE 'FAIL' END
  UNION ALL SELECT 63, '039', 'payroll_period_entries_pay_mode_check allows manual',
                   (SELECT pg_get_constraintdef(oid) FROM pg_constraint
                    WHERE conrelid = to_regclass('public.payroll_period_entries') AND conname = 'payroll_period_entries_pay_mode_check'),
                   CASE WHEN EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = to_regclass('public.payroll_period_entries')
                      AND conname = 'payroll_period_entries_pay_mode_check' AND pg_get_constraintdef(oid) LIKE '%manual%') THEN 'PASS' ELSE 'FAIL' END
  UNION ALL SELECT 64, '039', 'manual_compensations rows (expected 0 right after deploy)',
                   (SELECT count(*) FROM manual_compensations)::text, 'INFO'

  UNION ALL SELECT 20, 'CAA', 'company matches for name ILIKE ''%CAA%''',
                   (SELECT count(*) FROM caa)::text || ' → ' || coalesce((SELECT string_agg(name || ' [' || id || ']', '; ') FROM caa), 'none'),
                   CASE WHEN (SELECT count(*) FROM caa) = 1 THEN 'PASS' ELSE 'STOP: expected exactly 1 CAA company' END
  UNION ALL SELECT 21, 'CAA', 'company record',
                   (SELECT jsonb_build_object('name', c.name, 'language', c.language, 'created_at', c.created_at,
                      'subscription_status', to_jsonb(c)->>'subscription_status')::text
                    FROM companies c WHERE c.id = (SELECT id FROM caa LIMIT 1)), 'INFO'
  UNION ALL SELECT 22, 'CAA', 'company_document_settings (raw row; null = no row yet)',
                   (SELECT (to_jsonb(s) - 'id' - 'logo_storage_path' - 'document_header' - 'document_footer' - 'updated_at')::text
                    FROM company_document_settings s WHERE s.company_id = (SELECT id FROM caa LIMIT 1)), 'INFO'
  UNION ALL SELECT 23, 'CAA', 'profiles total', (SELECT count(*) FROM profiles WHERE company_id IN (SELECT id FROM caa))::text, 'INFO'
  UNION ALL SELECT 24, 'CAA', 'profiles by role / status / auth_status',
                   (SELECT string_agg(k || ': ' || n, '; ' ORDER BY k) FROM (
                      SELECT role || '/' || coalesce(status,'-') || '/' || coalesce(auth_status,'-') AS k, count(*) AS n
                      FROM profiles WHERE company_id IN (SELECT id FROM caa) GROUP BY 1) x), 'INFO'
  UNION ALL SELECT 25, 'CAA', 'profiles with a password set (count only)',
                   (SELECT count(*) FILTER (WHERE password_hash IS NOT NULL AND password_hash <> '') || ' of ' || count(*)
                    FROM profiles WHERE company_id IN (SELECT id FROM caa)), 'INFO'
  UNION ALL SELECT 26, 'CAA', 'workers total / active', (SELECT count(*) || ' / ' || count(*) FILTER (WHERE status = 'active')
                    FROM workers WHERE company_id IN (SELECT id FROM caa)), 'INFO'
  UNION ALL SELECT 27, 'CAA', 'active people by rate type (input to 037 Pay System inference)',
                   (WITH people AS (
                      SELECT (daily_rate IS NOT NULL AND daily_rate > 0) AS d, (hourly_rate IS NOT NULL AND hourly_rate > 0) AS h
                      FROM profiles WHERE role = 'employee' AND status = 'active' AND company_id IN (SELECT id FROM caa)
                      UNION ALL
                      SELECT (daily_rate IS NOT NULL AND daily_rate > 0), (hourly_rate IS NOT NULL AND hourly_rate > 0)
                      FROM workers WHERE status = 'active' AND company_id IN (SELECT id FROM caa))
                    SELECT 'daily=' || count(*) FILTER (WHERE d) || ', hourly=' || count(*) FILTER (WHERE h) || ', total=' || count(*)
                      || ' → 037 inference: pay_system = ' ||
                      CASE WHEN count(*) > 0 AND count(*) FILTER (WHERE d) = count(*) AND count(*) FILTER (WHERE h) = 0 THEN '''daily'''
                           WHEN count(*) > 0 AND count(*) FILTER (WHERE h) = count(*) AND count(*) FILTER (WHERE d) = 0 THEN '''hourly'''
                           ELSE 'NULL (mixed or no rates; admin chooses in Settings)' END
                    FROM people), 'INFO'
  UNION ALL SELECT 28, 'CAA', 'projects', (SELECT count(*) FROM projects WHERE company_id IN (SELECT id FROM caa))::text, 'INFO'
  UNION ALL SELECT 29, 'CAA', 'time_entries total', (SELECT count(*) FROM time_entries WHERE company_id IN (SELECT id FROM caa))::text, 'INFO'
  UNION ALL SELECT 30, 'CAA', 'time_entries September 2026 (1–15 / 16–30 / total)',
                   (SELECT count(*) FILTER (WHERE clock_in <  '2026-09-16') || ' / ' ||
                           count(*) FILTER (WHERE clock_in >= '2026-09-16') || ' / ' || count(*)
                    FROM time_entries WHERE company_id IN (SELECT id FROM caa)
                      AND clock_in >= '2026-09-01' AND clock_in < '2026-10-01'), 'INFO'
  UNION ALL SELECT 31, 'CAA', 'expenses', (SELECT count(*) FROM expenses WHERE company_id IN (SELECT id FROM caa))::text, 'INFO'
  UNION ALL SELECT 32, 'CAA', 'mileage_trips',
                   (SELECT count(*) FROM mileage_trips WHERE company_id IN (SELECT id FROM caa))::text, 'INFO'
