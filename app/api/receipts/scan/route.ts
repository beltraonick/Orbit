...
If a field cannot be determined, use null. Amounts should be numbers (no currency symbols).`
  ...
  for (const model of GROQ_MODELS) {
    try {
      ...
      console.error(`[receipts/scan] model ${model} failed:`, lastError)
      // If model is decommissioned or not found, try next; otherwise stop
      if (!lastError.includes('decommissioned') && !lastError.includes('model_not_found') && !lastError.includes('not found') && !lastError.includes('not_found')) {
        break
      }
    }
  }

  return Response.json({ error: 'scan_failed', message: lastError }, { status: 200 })
}
